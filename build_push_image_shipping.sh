...
# Build image from Docker file with var $ECOMMERCE_SHIPPING_REPO_NAME and tag latest
# You can see it from .env configuration
docker build --pull --rm -f "Dockerfile" -t $ECOMMERCE_SHIPPING_REPO_NAME:latest "."

# Show all list of docker iamge
docker image ls

# Define full new image name, with registry, namespace, repo
FULL_NEW_IMAGE_NAME=$GITHUB_REGISTRY/$GITHUB_USERNAME/$ECOMMERCE_SHIPPING_REPO_NAME:latest

# Rename previous image repo with new repo and tag
docker tag $ECOMMERCE_SHIPPING_REPO_NAME:latest $FULL_NEW_IMAGE_NAME
